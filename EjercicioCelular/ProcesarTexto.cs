...
{
    public class ProcesarTexto //Armo la Clase Publica
    {
        public String teclas(String texto) //armo una clase de String para el texto
        {
            String secuencia = ""; // declaro mi variable
            String aux = texto.ToUpper(new CultureInfo("en-US", false)); // declaro una nueva Cultura,  y to upper para may/minus
            char letraAnterior = (char)32;

            foreach (char c in aux) // ejecuto la declaracion
            {
                if (letraAnterior == c)
                {
                    secuencia = secuencia + " ";
                }
                //armo los casos a traves de los numeros, y lo complemento con un switch
                //NUMERO 2 
                if (c == 'A' || c == 'B' || c =='C')
                {
                    switch (c) {
